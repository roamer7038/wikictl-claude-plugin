...
3. Existing page: `wikictl cat --json <path>` prints `content` and `sha`, or `content_base64` instead of `content` when the file is not valid UTF-8, which is decoded before editing. Write `content` to a temporary file, edit it, then `wikictl put --json --base <sha> -m <reason> <path> < page.md`. The `sha` printed by `put --json` is valid for the next `--base`.
